...
import { spacesApi, resourcesApi, membersApi, type Space, type Resource } from '../api/client';
...
export function TenantPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    spacesApi.getById(slug)
      .then(async (s) => {
        setSpace(s);
        try {
          const res = await resourcesApi.getAll(s.id);
          setResources(res);
        } catch {
  ...
  async function handleLeave() {
    if (!space) return;
    try {
      await membersApi.leave(space.id);
      navigate('/spaces');
    } catch {
  ...
  if (error || !space) return <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-sm mb-6">⚠️ {error ?? 'Space not found.'}</div>;
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-wrap items-start justify-between gap-4 mb-8 max-md:flex-col">
        ...
        <div className="flex gap-2 flex-wrap max-md:w-full">
          {isOwner && (
            <button className="bg-white hover:bg-slate-50 text-slate-700 font-semibold px-5 py-2.5 rounded-xl border border-slate-200 transition-colors cursor-pointer min-h-[44px] max-md:w-full" onClick={() => navigate(`/spaces/${slug}/settings`)}>
              ⚙️ Settings
            </button>
          )}
          {!isOwner && (
        ...
        <div className="text-center py-16">
          ...
          {isOwner ? (
            <p className="text-slate-500">
              Add resources in{' '}
              <button className="bg-transparent border-none text-indigo-600 cursor-pointer text-[inherit] underline p-0" onClick={() => navigate(`/spaces/${slug}/settings`)}>settings</button>.
            </p>
          ) : (
          ...
          {resources.map(r => (
            <div key={r.id} className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-5 flex gap-5 items-center transition-all hover:shadow-md max-md:flex-col max-md:items-start">
              ...
              <button
                ...
                onClick={() => navigate(`/spaces/${slug}/resources/${r.id}`)}
              >
                Book now
              </button>
            </div>
      ...
      {confirmLeave && (
        ...
        >
          <p className="text-sm text-slate-500">You will lose your membership in <span className="font-semibold text-slate-700">{space.name}</span>.</p>
        </ConfirmDialog>
      )}
    </div>
  );
